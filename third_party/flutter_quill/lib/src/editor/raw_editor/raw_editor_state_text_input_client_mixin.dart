...
mixin RawEditorStateTextInputClientMixin on EditorState
  ...
  void updateRemoteValueIfNeeded() {
    if (!hasConnection || _receivingRemoteValue) {
      return;
    }
    ...
    final actualValue = value.copyWith(
      ...
      composing: composingRange.end > value.text.length ||
          value.text != _lastKnownRemoteTextEditingValue!.text
          ? TextRange.empty : composingRange,
    );
  ...
  bool _receivingRemoteValue = false;
  @override
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    // Quill requires a terminal newline. Native replacement/IME updates may
    // omit it; never diff away the last document node while inserting text.
    if (!value.text.endsWith('\n')) {
      value = value.copyWith(text: '${value.text}\n');
    }
