...
class RenderParagraphProxy extends RenderProxyBox
  ...
  @override
  Offset getOffsetForCaret(TextPosition position, Rect caretPrototype) {
    final caret = child!.getOffsetForCaret(position, caretPrototype);
    final length = child!.text.toPlainText().length;
    // Tight glyph boxes already include baseline placement and line leading.
    // Do not add a second estimate based on preferredLineHeight.
    final end = position.offset.clamp(0, length);
    var boxes = child!.getBoxesForSelection(
      TextSelection(baseOffset: 0, extentOffset: end),
      boxHeightStyle: BoxHeightStyle.tight,
    );
    if (boxes.isEmpty && length > 0) {
      boxes = child!.getBoxesForSelection(
        TextSelection(baseOffset: 0, extentOffset: length),
        boxHeightStyle: BoxHeightStyle.tight,
      );
      if (boxes.isNotEmpty) boxes = [boxes.first];
    }
    // At a soft-wrap boundary use the box on the caret's visual line.
    final candidate = boxes.isNotEmpty ? boxes.last : null;
    final box = candidate != null && caret.dy < candidate.bottom
        ? candidate
        : null;
    final center = box == null
        ? caret.dy +
              (child!.getFullHeightForCaret(position) ?? preferredLineHeight) /
                  2
        : (box.top + box.bottom) / 2;
    return Offset(
      caret.dx,
      center - caretPrototype.height / 2 - caretPrototype.top,
    );
  }
