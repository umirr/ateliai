...
class EditableTextBlock extends StatelessWidget {
  ...
  List<Widget> _buildChildren(
    ...
    for (final line in Iterable.castFrom<dynamic, Line>(block.children)) {
      ...
      final firstSize =
          ...
          defaultStyles?.paragraph?.style.fontSize ??
          24;
      final defaultIndent = indentWidthBuilder(
        block,
        context,
        count,
        numberPointWidthBuilder,
      );
      final requiredIndent =
          numberPointWidthBuilder(firstSize, count) +
          firstSize *
              ((line.style.attributes[Attribute.indent.key]?.value as num?) ??
                  0);
      final lineIndent =
          line.style.attributes.containsKey(Attribute.list.key) &&
              requiredIndent > defaultIndent.left
          ? defaultIndent.copyWith(left: requiredIndent)
          : defaultIndent;
  ...
  Widget? _buildLeading({
    ...
    final leadingConfig = LeadingConfig(
      ...
      style: () {
        if (isOrdered) {
          return (defaultStyles.lists?.style ?? defaultStyles.leading!.style)
              .copyWith(fontSize: size ?? fontSize, color: fontColor);
        }
        if (isUnordered) {
          return (defaultStyles.lists?.style ?? defaultStyles.leading!.style)
              .copyWith(
      ...
      lineSize: isCheck ? (size ?? fontSize) : null,
