...
class _TextLineState extends State<TextLine> {
  ...
  InlineSpan _getTextSpanForWholeLine() {
    ...
    // Hash headings are visual only: retain literal characters, selection and IME offsets.
    final hashes = RegExp(r'^(#+)').firstMatch(widget.line.toPlainText());
    if (hashes != null) {
      final n = hashes[1]!.length;
      const colors = [
        Color(0xffb88ae8),
        Color(0xffed7777),
        Color(0xff72c698),
      ...
      if (Theme.of(context).brightness == Brightness.light) {
        color = HSLColor.fromColor(color).withLightness(.36).toColor();
      }
      lineStyle = lineStyle.copyWith(
        fontWeight: FontWeight.bold,
        color: color,
        fontSize: 32 - math.min(n - 1, 5) * 2.0,
      );
    }
...
class RenderEditableTextLine extends RenderEditableBox {
  ...
  double get cursorHeight {
    if (cursorCont.style.height != null) return cursorCont.style.height!;
    final body = _body;
    if (body is RenderParagraphProxy && body.child is RenderParagraph) {
      final paragraph = body.child! as RenderParagraph;
      final at = (textSelection.extentOffset - line.documentOffset - 1).clamp(
        0,
        math.max(0, line.length - 2),
      );
      final span = paragraph.text.getSpanForPosition(
        TextPosition(offset: at.toInt()),
      );
      final size =
          span?.style?.fontSize ?? paragraph.text.style?.fontSize ?? 24;
      return paragraph.textScaler.scale(size) * 1.15;
    }
    return preferredLineHeight(const TextPosition(offset: 0));
  }
  ...
  void performLayout() {
    ...
    if (_leading != null) {
      ...
      // Storyloom: list markers follow the actual first glyph for centered/right
      // paragraphs. Leave document offsets and caret geometry unchanged.
      var markerX = 0.0;
      final alignment = line.style.attributes[Attribute.align.key]?.value;
      if (textDirection == TextDirection.ltr &&
          (alignment == 'center' || alignment == 'right')) {
        final boxes = _body!.getBoxesForSelection(
          TextSelection(
            baseOffset: 0,
            extentOffset: math.max(0, line.length - 1),
          ),
        );
        if (boxes.isNotEmpty) markerX = boxes.first.left;
      }
      var markerY = 0.0;
      final type = line.style.attributes[Attribute.list.key]?.value;
      final isCheck = type == 'checked' || type == 'unchecked';
      if (isCheck) {
        final boxes = _body!.getBoxesForSelection(
          TextSelection(
            baseOffset: 0,
        ...
        if (boxes.isNotEmpty) {
          markerY =
              (boxes.first.top + boxes.first.bottom - _leading!.size.height) /
              2;
        }
      } else {
        final baseline = _body!.getDistanceToBaseline(
          TextBaseline.alphabetic,
          onlyReal: true,
        );
        final markerBaseline = _leading!.getDistanceToBaseline(
          TextBaseline.alphabetic,
          onlyReal: true,
        );
        if (baseline != null && markerBaseline != null)
          markerY = baseline - markerBaseline;
      }
      (_leading!.parentData as BoxParentData).offset = Offset(
        markerX,
        _resolvedPadding!.top + markerY,
      );
    }
